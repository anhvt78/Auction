...
export default function DisputeDetail({ lang = "vi", onBack }) {
  ...
  const [d, setD] = useState({
    ...
    deadline: {
      ...
      actionRequired: isVi
        ? "Cung cấp video đóng gói hàng"
        : "Provide packaging video",
    ...
    auction: {
      id: "AUC-10293",
      title: isVi
        ? "Lô thiết bị văn phòng Dell Precision - 10 món"
        : "Dell Precision Office Set - 10 items",
      finalBid: "2,500 BBD",
    ...
    shipping: {
      ...
      history: [
        {
          time: "19/03/2026 15:40",
          note: isVi ? "Giao hàng thành công" : "Delivered successfully",
        },
        {
          time: "17/03/2026 10:20",
          note: isVi ? "Đã lấy hàng từ người bán" : "Picked up from seller",
        },
      ],
    },
    ...
    messages: [
      {
        ...
        role: "buyer",
        content: isVi
          ? "Tôi nhận hàng thấy 2 máy in bị nứt vỏ nhựa."
          : "I received items with cracked casings.",
  ...
  if (!d || !d.shipping || !d.auction) {
    return (
      <div className="p-10 text-center opacity-50 italic">Loading data...</div>
    );
  }
        ...
        <div className="bg-red-600 text-white p-4 rounded-sm shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 animate-pulse">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-black text-sm uppercase tracking-wider">
                {isVi
                  ? "HÀNH ĐỘNG CỦA BẠN LÀ BẮT BUỘC"
                  : "YOUR ACTION IS REQUIRED"}
              </h3>
              <p className="text-[11px] opacity-90 font-medium italic">
                {isVi
                  ? `Nhiệm vụ: ${d.deadline.actionRequired}`
                  : `Task: ${d.deadline.actionRequired}`}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-end border-l border-white/20 pl-6">
            ...
            <p className="text-[9px] font-bold bg-black/20 px-2 py-0.5 mt-1 rounded-sm text-red-100">
              *{" "}
              {isVi
                ? `Quá hạn: ${d.deadline.consequence}`
                : `Expired: ${d.deadline.consequence}`}
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 border border-slate-200 rounded-sm shadow-sm">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-4 text-xs text-slate-500">
            ...
            <span className="flex items-center gap-1">
              <User size={14} /> {isVi ? "Người mua:" : "Buyer:"}{" "}
              {d.auction.buyer}
            </span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="text-[11px] font-bold text-blue-600 bg-blue-50 px-3 py-1 border border-blue-100 flex items-center gap-2 italic">
            <ShieldCheck size={14} /> {isVi ? "Tiền đang tạm giữ:" : "Escrow:"}{" "}
            {d.financials.escrowAmount}
          </div>
        </div>
      </div>
          ...
          <section className="bg-white border border-slate-200 rounded-sm overflow-hidden shadow-sm">
            <div className="bg-slate-800 px-4 py-2 flex justify-between items-center">
              <h3 className="text-[10px] font-bold text-white uppercase tracking-widest flex items-center gap-2">
                ...
                {isVi ? "Thông tin đấu giá gốc" : "Auction Reference"}
              </h3>
              <a
                href={d.auction.link}
                className="text-[10px] font-bold text-blue-400 flex items-center gap-1 hover:text-white transition-colors"
              >
                {isVi ? "CHI TIẾT PHIÊN" : "VIEW AUCTION"}{" "}
            ...
            <div className="p-5 grid grid-cols-2 md:grid-cols-3 gap-6">
              ...
              <div>
                <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">
                  {isVi ? "Giá thắng thầu" : "Final Price"}
                </p>
              ...
              <div>
                ...
                <p className="text-sm font-bold text-blue-600 underline cursor-pointer">
                  @{d.auction.seller}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-6">
                <div className="bg-slate-50 p-6 border border-slate-200 rounded-sm">
                  <label className="block text-[10px] font-black uppercase text-slate-500 mb-3">
                    {isVi
                      ? "Tải lên tệp bằng chứng (Video/Ảnh)"
                      : "Upload Evidence"}
                  </label>
                  <div className="border-2 border-dashed border-slate-300 rounded-sm p-10 flex flex-col items-center justify-center hover:bg-white hover:border-blue-400 transition-all cursor-pointer group">
                    ...
                    />
                    <p className="text-xs font-bold text-slate-500 group-hover:text-blue-600 underline uppercase">
                      {isVi ? "Chọn tệp hoặc kéo thả" : "Drag & Drop Files"}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder={
                      isVi
                        ? "Mô tả rõ ràng về bằng chứng bạn cung cấp..."
                        : "Describe your evidence clearly..."
                    }
                  />
                </div>
          ...
          <section className="bg-white border border-slate-200 p-6 rounded-sm shadow-sm opacity-90">
            ...
            <div className="space-y-8 relative">
              {d.messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`flex ${msg.role === "admin" ? "justify-center" : msg.role === "buyer" ? "justify-start" : "justify-end"}`}
                >
                  <div
                    className={`relative max-w-[85%] p-4 rounded-sm border ${
                      msg.role === "admin"
                  ...
                  >
                    <div
                      className={`flex items-center gap-3 mb-2 text-[10px] font-bold uppercase tracking-tighter ${msg.role === "admin" ? "justify-center" : "justify-between"}`}
                    >
                      <span
                        className={
                          msg.role === "admin"
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end border-b pb-2">
                <span className="text-xs text-slate-400 font-bold">
                  {isVi ? "Giá trị khiếu nại:" : "Claim Amount:"}
                </span>
                <span className="text-lg font-black text-red-600">
                  {d.financials.claimAmount}
                </span>
              </div>
          ...
          <section className="bg-slate-900 text-white p-6 rounded-sm shadow-xl relative overflow-hidden">
            ...
            <h4 className="text-[11px] font-black uppercase mb-4 flex items-center gap-2 tracking-[0.2em]">
              ...
              {isVi ? "Quy tắc Trọng tài" : "Arbitration Rules"}
            </h4>
            <ul className="text-[10px] space-y-3 opacity-80 leading-relaxed font-medium">
              ...
              <li className="flex gap-2">
                ...
                {isVi
                  ? "Bằng chứng video phải nguyên vẹn."
                  : "Video evidence must be intact."}
              </li>
            </ul>
          </section>
        </div>
